...
mod tests {
    ...
    #[test]
    fn test_get_remote_info_from_repo_root() {
        let (_tmp, repo_path) = create_temp_repo_with_remote();
        let result = get_remote_info(&repo_path).unwrap();
        assert_eq!(result.0, "example");
        assert_eq!(result.1, "cosmos");
    }

    #[test]
    fn test_get_remote_info_from_subdirectory() {
        let (_tmp, repo_path) = create_temp_repo_with_remote();
        let nested = repo_path.join("src").join("nested");
        std::fs::create_dir_all(&nested).unwrap();

        let result = get_remote_info(&nested).unwrap();
        assert_eq!(result.0, "example");
        assert_eq!(result.1, "cosmos");
    }

    fn create_temp_repo_with_remote() -> (tempfile::TempDir, PathBuf) {
        let temp_dir = tempfile::tempdir().unwrap();
        let repo_path = temp_dir.path().to_path_buf();
        let repo = Repository::init(&repo_path).unwrap();
        let _ = repo
            .remote("origin", "https://github.com/example/cosmos.git")
            .unwrap();
        (temp_dir, repo_path)
    }
}
